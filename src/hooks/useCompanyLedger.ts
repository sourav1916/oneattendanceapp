...
import { fetchCompanyLedger } from '@src/api/fetchCompanyLedger';
import type {
  CompanyLedgerMeta,
  LedgerTransaction,
...
export type UseCompanyLedgerOptions = {
...
export function useCompanyLedger({
  ...
  const loadFirst = useCallback(async () => {
    ...
    try {
      const res = await fetchCompanyLedger(companyId, {
        page_no: 1,
        limit: PAGE_SIZE,
      ...
      setOpeningBalance(hasDateRange ? (res.data?.opening_balance ?? 0) : null);
      setMeta(res.meta);
